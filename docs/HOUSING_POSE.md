...
chassis-tag yaw cannot rotate the reconstructed robot. The final solve fits
full-resolution image corners, registered LiDAR tag-plane range, surveyed floor
coordinates, and small whole-robot movement between reconnect segments. One
floor tag names the output origin. BuildViz initializes the correct leg/face
branches and is then reported only as a diagnostic comparison, not imposed as
ground truth. Detected-versus-predicted overlays make the result visually
auditable. See
