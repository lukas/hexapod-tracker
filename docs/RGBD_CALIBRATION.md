...
## Handheld zero-pose tag survey (removed 2026-09-12)

The iPhone walk-around survey (`zero_pose_survey`, `zero_pose_refinement`,
`tag_survey`, the web studio) was superseded by `hexapod-calibrate-tags`,
which needs no phone, and deleted on 2026-09-12; see `DEPRECATED.md`. Its
replayable RGB-D frame archive format is still written by
`rgbd_calibrate.archive_frame` and read back by `hexapod-rgbd-calibrate`.
...
- `camera_pose_xyzw_xyz`: written for the removed zero-pose survey and kept for replay; Record3D's
