...
function ZeroSurveyWorkspace({
  ...
  const refinement = survey?.buildviz_refinement
  const maxReconnectMotionMm = Math.max(
    0,
    ...(refinement?.scene_motion?.segments.map((item) => item.translation_mm) || []),
  )
  const auditFrames = refinement?.visual_reprojection_audit?.overlay_frames?.slice(0, 4) || []
    ...
    <main className="calibration-app">
      ...
      <div className="survey-shell">
        <aside className="wizard-rail">
          <div className="rail-intro"><span>Guided setup</span><b>Take your time · progress is saved</b></div>
          {[
        ...
        <section className="survey-main">
            ...
            <>
              <div className="live-grid">
                <section className="camera-card">
                  ...
                  <div className="camera-stats"><span><b>{survey.frame_sequence}</b> camera frames drained</span><span><b>{survey.detected_tag_ids.length}</b> tags in last analysis</span><span><b>{survey.elapsed_s.toFixed(0)}s</b> elapsed</span></div>
          ...
          {(complete || survey?.status === 'incomplete') && survey && (
            <section className={`review-card ${complete ? 'complete' : 'partial'}`}>
              ...
              {refinement && <div className={`geometry-gate ${refinement.ok ? 'passed' : 'checking'}`}><div><b>{refinement.ok ? '3-D reconstruction verified' : 'Reconstruction needs another scan'}</b><span>{refinement.ok ? `${refinement.robot_tag_count} robot tags were fit against the original full-resolution photos, LiDAR range, and floor landmarks. Green/magenta overlays below compare the reconstruction directly with the photos.` : refinement.skipped_reason}</span></div><div><span><b>{refinement.pre_bundle_coordinate_rms_px?.toFixed(2)} → {refinement.final_coordinate_rms_px?.toFixed(2)} px</b>accepted image coordinates</span><span><b>{refinement.all_observation_coordinate_rms_px?.toFixed(2) ?? '—'} px</b>all observations</span><span><b>{refinement.lidar_depth?.median_absolute_error_mm?.toFixed(1) ?? '—'} mm</b>median LiDAR range error</span><span><b>{refinement.lidar_depth?.p90_absolute_error_mm?.toFixed(1) ?? '—'} mm</b>90th percentile LiDAR</span><span><b>{((refinement.scene_motion?.floor_rejection_fraction || 0) * 100).toFixed(1)}%</b>floor views rejected</span><span><b>{maxReconnectMotionMm.toFixed(1)} mm</b>largest reconnect movement</span><span><b>{refinement.median_shared_mount_deviation_mm?.toFixed(1)} mm</b>median BuildViz disagreement</span></div>{auditFrames.length > 0 && <figure className="reprojection-audit"><figcaption>Photo check: detected green · predicted magenta · yellow line is the error</figcaption><div>{auditFrames.map((item) => { const filename = item.path.split('/').pop(); return <a key={item.frame} href={`/api/vision/zero-survey/reprojection-audit/${filename}`} target="_blank" rel="noreferrer"><img src={`/api/vision/zero-survey/reprojection-audit/${filename}`} alt={`Reprojection check for ${item.frame}`} /><span>{item.coordinate_rms_px?.toFixed(2) ?? '—'} px</span></a> })}</div></figure>}{refinement.floor_reference?.uses_measured_ground_truth ? <small>Floor-tag coordinates are active metric anchors, with their configured measurement uncertainty.</small> : <small>The floor map is provisional. This pass remeasured it from RGB + LiDAR; surveyed tag-center coordinates will tighten the metric reference further.</small>}{maxReconnectMotionMm > 5 && <small>The robot appears to have moved during a reconnect; that rigid movement was estimated separately instead of distorting the tag layout.</small>}{Boolean(refinement.diagnostics?.floor_tags_moved_over_10mm?.length) && <small>Floor positions corrected by more than 10 mm: {refinement.diagnostics?.floor_tags_moved_over_10mm?.map((id) => `#${id}`).join(', ')}</small>}{Boolean(refinement.diagnostics?.weak_viewpoint_geometry_tag_ids?.length) && <small>Tags that still need a wider viewing angle: {refinement.diagnostics?.weak_viewpoint_geometry_tag_ids?.map((id) => `#${id}`).join(', ')}</small>}{Boolean(refinement.diagnostics?.buildviz_or_mount_mismatch_tag_ids.length) && <small>Image-consistent tags that disagree with BuildViz/mount assumptions: {refinement.diagnostics?.buildviz_or_mount_mismatch_tag_ids.map((id) => `#${id}`).join(', ')}</small>}</div>}
