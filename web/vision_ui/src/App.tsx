        ...
        <section className="survey-main">
          ...
          {(complete || survey?.status === 'incomplete') && survey && (
            <section className={`review-card ${complete ? 'complete' : 'partial'}`}>
              ...
              {complete && survey.buildviz_refinement?.ok && <div className="geometry-gate passed"><div><b>Photo bundle adjustment passed</b><span>{survey.buildviz_refinement.robot_tag_count} robot tags across {survey.buildviz_refinement.frames} archived full-resolution views were fit directly to decoded image corners. One floor tag fixes the origin; the other floor tags are measured from the photos. BuildViz is only a comparison.</span></div><div><span><b>{survey.buildviz_refinement.pre_bundle_coordinate_rms_px?.toFixed(2)} → {survey.buildviz_refinement.final_coordinate_rms_px?.toFixed(2)} px</b>all corner coordinates</span><span><b>{survey.buildviz_refinement.floor_coordinate_rms_px?.toFixed(2)} px</b>measured floor fit</span><span><b>{survey.buildviz_refinement.robot_coordinate_rms_px?.toFixed(2)} px</b>robot image fit</span><span><b>{survey.buildviz_refinement.median_shared_mount_deviation_mm?.toFixed(1)} mm</b>median BuildViz disagreement</span></div>{Boolean(survey.buildviz_refinement.diagnostics?.floor_tags_moved_over_10mm?.length) && <small>Loose floor positions corrected by more than 10 mm: {survey.buildviz_refinement.diagnostics?.floor_tags_moved_over_10mm?.map((id) => `#${id}`).join(', ')}</small>}{Boolean(survey.buildviz_refinement.diagnostics?.weak_viewpoint_geometry_tag_ids?.length) && <small>Tags that still need a wider viewing angle: {survey.buildviz_refinement.diagnostics?.weak_viewpoint_geometry_tag_ids?.map((id) => `#${id}`).join(', ')}</small>}{Boolean(survey.buildviz_refinement.diagnostics?.buildviz_or_mount_mismatch_tag_ids.length) && <small>Image-consistent tags that disagree with BuildViz/mount assumptions: {survey.buildviz_refinement.diagnostics?.buildviz_or_mount_mismatch_tag_ids.map((id) => `#${id}`).join(', ')}</small>}</div>}
